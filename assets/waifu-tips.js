...
live2d_settings['defaultModel'] =
	...
	.replace(/MODEL_HOME/g, live2d_settings['staticPath']);
...
var staticAPI;
$.ajax({
	url: live2d_settings.staticAPIFile,
	methods: "get",
	async: false,
...
function initModel(waifuPath, type) {
	...
	live2d_settings.homePageUrl == 'auto' ? window.location.protocol + '//' + window.location.hostname + '/' :
		live2d_settings.homePageUrl;
	if (window.location.protocol == 'file:' && live2d_settings.modelAPI.substr(0, 2) == '//') live2d_settings.modelAPI =
	...
	if (typeof(waifuPath) == "object") loadTipsMessage(waifuPath);
		...
		$.ajax({
			cache: true,
			url: waifuPath == '' ? live2d_settings.tipsMessage : (waifuPath.substr(waifuPath.length - 15) == 'waifu-tips.json' ?
...
function loadModel(modelId, modelTexturesId = 0) {
	...
	loadlive2d('live2d', localAPI('get', modelId, modelTexturesId), (live2d_settings.showF12Status ? console.log(
		'[Status]', 'live2d', '模型', modelId + '-' + modelTexturesId, '加载完成') : null));
}
		...
		if (window.location.href == live2d_settings.homePageUrl) {
			...
			if (document.referrer !== '') {
				...
				if (window.location.hostname == referrer.hostname)
					text = referrer_message.localhost[0] + document.title.split(referrer_message.localhost[2])[0] + referrer_message.localhost[
						1];
					...
					text = referrer_message.google[0] + document.title.split(referrer_message.google[2])[0] + referrer_message.google[
						1];
				else {
			...
			} else text = referrer_message.none[0] + document.title.split(referrer_message.none[2])[0] + referrer_message.none[
				1];
		}
